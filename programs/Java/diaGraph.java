...
public class diaGraph  extends  Applet  implements  ActionListener
{
	...
	private Label name_str = new Label("名前");
	private Label eng_str = new Label("英語");
	private Label jap_str = new Label("国語");
	private Label math_str = new Label("数学");
	private Label sci_str = new Label("理科");
...
private Color[] colors = {Color.red, Color.blue, Color.yellow, Color.green, Color.black, Color.gray, Color.pink};
	...
	public void paint (Graphics g)
	{
		...
		g.drawString("英語", x0+20, y0-(100*val)+2);
		g.drawString("国語", x0-(100*val)-25, y0);
		g.drawString("数学", x0+20, y0+(100*val)+2);
		g.drawString("理科", x0+(100*val)+10, y0);
